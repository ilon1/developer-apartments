...
class DEV_Map_Module_V2 extends ET_Builder_Module {
    ...
    public function get_fields(){
        ...
        return array(
            ...
            'source_id'     => array(
                'label'=>'Zdroj (projekt/budova/podlažie)',
                'type'=>'select',
                'options'=>$opts,
            ...
            'overlay_color_mode' => array(
                'label'=>'Farba overlay',
                'type'=>'select',
                'options'=>array('shape'=>'Podľa shape','override'=>'Vynútiť farbu'),
                'default'=>'shape',
                'tab_slug'=>'general',
                'toggle_slug'=>'style'
            ),
            ...
            'tooltip_size'  => array(
                'label'=>'Tooltip: veľkosť písma (px)',
                'type'=>'text',
                'default'=>'14',
                'tab_slug'=>'general',
                'toggle_slug'=>'tooltip'
            ...
            'overlay_click_action' => array(
                'label'=>'Klik na overlay',
                'type'=>'select',
                'options'=>array(
                    'navigate'=>'Prejsť na kategóriu',
                    'emit_event'=>'Poslať event do stránky'
                ),
                'default'=>'navigate',
                'tab_slug'=>'general',
    ...
    private function resolve_source_term(){
        ...
        $raw    = $this->props['source_id'] ?? '';
        $manual = intval($raw);

        // HOTFIX 1 – manuál má prednosť, ak nie je prázdny
        if($raw !== '' && $manual > 0){
            return $manual;
        }

        // HOTFIX 2 – ak je režim manual, ale bez hodnoty → nevybraný zdroj
        if($mode === 'manual'){
            return 0;
        }

        // HOTFIX 3 – Theme Builder context
    ...
    private function builder_preview($h, $src){
        ...
        <div class="dev-map-preview"
             style="height:<?php echo esc_attr($h); ?>px;
                    display:flex;align-items:center;justify-content:center;
                    background:#f6f7f9;border:1px dashed #cdd3db;border-radius:8px;">
    ...
    public function render($attrs,$content=null,$render_slug){
        ...
        // --- Enrich shapes: link + free_count (for term targets) ---
        $enriched = array();
        foreach ($shapes as $s) {
            $row = $s;
            ...
            if (!empty($s['target_type']) && $s['target_type'] === 'term' && !empty($s['target_id'])) {
                ...
                // Count of free apartments in the target term
        ...
        // --- CSS variables for tooltip & badge (themeable from module props) ---
        $css_vars = sprintf(
        ...
        // --- Payload for frontend JS (assets/js/map.js) ---
        $data = array(
            'termId'    => $term_id,
            'legend'    => $legend === '1',
            'shapes'    => $enriched,
        ...
        // --- Output wrapper (JS reads data-payload & data-img) ---
        $uid = 'dev-map-'.uniqid();

        ob_start(); ?>
        <div id="<?php echo esc_attr($uid); ?>"
             class="dev-apt-map"
             style="position:relative; height:<?php echo esc_attr(intval($h)); ?>px; <?php echo $css_vars; ?>"
